...
def main(main_disease_name, min_score, load_from_files=True):
    ...
    if load_from_files:
        ...
        df_merged_on_target.rename(columns={'id_x':'main_target_id',
					    'disease_id': 'main_disease_id', 
					    'id_y':'related_drug_id', 
					    'target_id':'related_target_id', 
					    'disease_id_x':'main_disease_id',
					    'disease_id_y':'related_disease_id'
					   }, inplace=True)

        df_merged_on_target.rename(columns={'disease_name':'releated_disease_name'})
        print(f'df_merged_on_target:\n\n{df_merged_on_target}')
        df_merged_on_target.to_csv("merged_on_target.csv", index=False)
    ...
    print(f"\nStep 1: Found diseases: {len(diseases)} for {main_disease_name}")
    diseases, disease_set = remove_diseases(diseases, disease_set, diseases_to_remove)
    print(f"\nStep 1: remove_diseases: {len(diseases)} for {main_disease_name}")
    ...
    print(f"\n\n{df}\n\n")
    df.to_csv("main_drugs.csv", index=False)
    ...
    print(f"\n\n{df}\n\n")
    df.to_csv("main_targets.csv", index=False)
    ...
    print(f"\n\n{df}\n\n")
    df.to_csv("related_diseases.csv", index=False)
    ...
    related_drug_info_df.to_csv("related_drugs_info.csv", index=False)
    ...
    candidate_drugs_df.to_csv("candidate_drugs.csv", index=False)
...
if __name__ == "__main__":
    ...
    min_score = 0.33
    main(main_disease_name, min_score, load_from_files=True)  # Set to True to load from files
